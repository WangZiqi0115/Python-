...
> 版本：v3，用户已决定跳过备份与配置迁移；包含驱动、软件分层、安全加固、回滚与验收协议。
...
用户于 2026-08-12 明确决定：不在本机备份任何内容，不做配置迁移；新机一律按全新环境配置。

该决定与“零、强制执行总则”同级，优先于本文件其它内容。因此：

- 所有“备份旧机数据”“迁移旧机配置”“整机映像备份”相关章节已标记为“已跳过”，仅保留为历史参考，不执行。
- 新机上的账号、SSH Key、AI 工具配置等全部重新登录、重新生成、重新设置。
- 只有已推送到远端仓库（GitHub / Gitee）的内容能自动在新机恢复；本机独有、未提交、未导出的文件不会迁移，本机一旦损坏即无法找回。

---

## 一、换机前备份与数据清单（旧电脑上做）【已跳过】

> 本节按用户决定跳过：不在本机备份任何内容。以下内容仅作历史存档参考，不执行。
...
系统还原点不足以保证驱动、UWP 应用和注册表深度改动绝对可逆，因此配置软件前必须先建立回滚底座。

> 用户已决定跳过备份：本节只保留系统还原点作为最低回滚保障，不创建整机映像。

1. 开启系统保护，并为 `C:` 创建还原点。
2. 整机系统映像：已按用户决定跳过，不创建。
3. 每次进入新阶段前再创建一个新的还原点。
4. 正式批量安装前，先测试一次还原流程，确认还原点可用。
5. 建立统一工作目录：
...
> 本节按用户决定跳过：新机 Codex 全新登录、全新配置，不复制旧机配置。以下内容仅作参考。
...
### 9.3 VS Code 设置迁移【已跳过】

> 本节按用户决定跳过：新机 VS Code 全新安装、手动恢复必要设置。以下内容仅作参考。
...
> 注意：已跳过本机备份，仓库内容以远端为唯一来源；clone 前先确认远端仓库包含需要保留的最新提交。
...
7. 如果用户要求彻底还原，使用安装前的系统还原点还原（整机映像已按用户决定跳过）。
...
- [ ] 文档、桌面、下载路径已迁移到数据盘
- [ ] 浏览器、微信、QQ 等登录态和数据迁移完成【已跳过，不验收】
- [ ] BitLocker 已开启
- [ ] Windows Defender 实时保护开启
- [ ] API Key / Token 已存入凭据管理器或密码管理器
- [ ] 一键清除协议测试通过
- [ ] 系统还原点验证可用（整机映像已按用户决定跳过）
