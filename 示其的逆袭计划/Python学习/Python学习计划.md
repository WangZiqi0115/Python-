...
> ⚠️ **重要（2026.9.5）**：本 50 天计划已被**重排取代**。
> 因目标改为「MNIST 手写数字识别」项目，现执行路线见
> [AI数字识别小项目.md](../AI数字识别小项目.md)（其标题即"Python 学习计划(重排·MNIST…)"）。
> **本文件只作历史路线回顾，不再按它推进。** 每日进度请到 AI数字识别小项目.md / [Python学习日志.md](Python学习日志.md) 查看。
...
| 21 | 8.16 | 异常进阶（✅ 已完成） | raise、assert、自定义异常、异常链、else/finally 深入 |
| 22 | 8.17 | 编码与 bytes（✅ 已完成） | ASCII/Unicode/UTF-8/GBK、str vs bytes、encode/decode、乱码原理 |
