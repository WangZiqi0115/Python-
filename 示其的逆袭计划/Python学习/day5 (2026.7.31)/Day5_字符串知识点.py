...
# 【补充】find 是"安全版"（找不到返回 -1），index 是"严格版"（找不到报错）
#   不确定子串是否存在 → 用 find 判断 != -1
#   确定一定存在 → 用 index
...
# 【补充】括号里的数字 = 最终总宽度，不够的部分用空格/零补齐
#   "hello".center(11)：总宽 11，"hello" 占 5，两边各补 3 个空格
#   如果指定宽度比原字符串还短，直接原样返回，不会截断
#   想截断长度要用切片："hello python"[:5]
...
# 【补充】这行在干嘛？
#   len(ids)=3 → ["?"]*3 → ["?", "?", "?"] → join → "?, ?, ?"
#   SQL 语句里用 ? 占位，实际值（1、2、3）单独作为参数传给数据库
#   不能把用户输入直接拼进 SQL（有注入风险），所以用占位符
